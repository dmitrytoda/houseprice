...
Creating new features can be done in two ways:

1. Create something intuitive, like Age that was created earlier
2. Create powers (usually squares) and interaction terms (products for numeric variables, categories like "has a pool AND a garage" for factors).

Before proceeding with the second part, I will have to drop redundant features to have less interaction terms and also to normalize (center and scale) the original features.

## Intuitive features {.tabset}
...
1. Keep only features that are confirmed or tentative, according to boruta
2. Remove features that are highly correlated with some other features (even if confirmed by boruta), based on the correlation table.
```{r}
...
Apply Box-Cox transformation to these features and check if it helped.
```{r}
for (i in skewed){
...
```
There are still 3 features with |skew|>.5, but it's not too bad.
...
## Square and interaction features

For all **true numeric features**, I will create 

1. Square features
2. Interaction terms (pair-wise products)

For all **non-ordered categories**, I will create interaction terms with a logical AND (e.g. is new and has a pool) which is same as multiplication for dummy variables. As a side effect, it will create a bunch of all-zero columns (all the conjunctions of different levels of the same features), which I will remove.

All the **ordered categories** are better left alone - no sense in squaring or multiplying them by each other.

This whole shebang may lead to overfitting but hopefully regularization will be able to deal with it.



```{r}
cuadratics <- sapply(numerics, function(x) x^2)
colnames(cuadratics) <- paste(colnames(numerics), '-sq', sep = '')
num_interactions <- model.matrix(~.^2-1, numerics)
cat_interactions <- model.matrix(~.^2-1, categorics)
zeroColInterac <- which(colSums(cat_interactions)==0)
cat_interactions <- cat_interactions[,-zeroColInterac]
```
...
2. cuadratics, num_interactions and cat_interactions contain squares and interactions
3. SalePrice is still a column in all, which has vertically stacked train and test sets (as do all other data frames in pp. 1-2)

I will combine them vertically, then split back into train and test horizontally.

```{r}
combined1 <- cbind(numerics, ordinals, categorics, cuadratics, num_interactions, cat_interactions)
train1 <- combined1[!is.na(all$SalePrice),]
test1 <- combined1[is.na(all$SalePrice),]
dim(train1)
```

3182 features! I better leave them models to train overnight.
...
It actually worked very fast. Lasso works great for when you have plenty of predictors by setting regression coefficients for many of them to zero. I want to see how many of the 3182 features were used.
...
write.csv(submit, 'lasso.csv', row.names = FALSE)
```

Leader board score of 0.12839 with lasso alone.
...
### Including quadratic and interaction terms - bad performance
<details>
  <summary>Click to expand!</summary>

This code is disabled as it takes a few hours on my laptop. The results are below.
...
However, caret optimization was run holding nrounds constant at 1000. It makes sense now to fix all other parameters to what caret produced, and tune the nrounds. This is done using native XGBoost interface, without caret.
...
```{r}
set.seed(11102020)
# train a model using caret parameters, with up to 1000
...
CV RMSE of XGBoost is 0.126278, which is significantly worse than 0.116289 reported by Erik in his original notebook. Looks like my quadratic features and/or interaction terms are not helping. Let's try again wihtouh them.
...
```{r}
combined2 <- cbind(numerics, ordinals, categorics)
...
```{r cache=TRUE, eval=FALSE}
xgb_caret <- train(x=train2, y=all$SalePrice[!is.na(all$SalePrice)], method='xgbTree', trControl= my_control, tuneGrid=xgb_grid) 
xgb_caret$bestTune
```