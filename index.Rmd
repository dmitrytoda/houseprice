...
2. Category variables (including ordinal and non-ordinal):
        + no centering or scaling
        + remove empty and almost-empty levels
        + for non-ordinal only, one-hot encode
...
ordinal_names <- c('LotShape', 'LandSlope', 'OverallQual', 'OverallCond', 'ExterQual', 'BsmtQual', 'BsmtCond', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'HeatingQC', 'KitchenQual', 'Functional', 'FireplaceQu', 'GarageFinish', 'GarageQual', 'PavedDrive', 'PoolQC')

# remove ordinals from numerics
numerics <- numerics[!(numerics %in% c(ordinal_names, 'SalePrice'))]

# get true unordered categories
categorics <- names(all)
categorics <- categorics[!(categorics %in% c(numerics, 'SalePrice'))]
...
skewed
```
28 skewed features. Apply Box-Cox transformation to these features and check if it helped.
```{r}
for (i in skewed){
...
```
There now 12 features with |skew|>.5 instead of 28. <span style="color: red;">This can definitely be improved</span>, but let's see, maybe it will work good enough like this.
...
### Removing zero variance and empty levels

1. For train set, if some categoric feature has 0 variance, it is useless for prediction.
2. For test set, if some dummy variable contains all zeros, it means this level is empty, and even if we have in train set, it is also useless. 

```{r}
# train set - remove zero variance
zeroVarTrain <- which(sapply(categorics[!is.na(all$SalePrice),], var)==0)

# test set - remove all-zero columns (all-one or all-some-ordinal-level may still be useful)
zeroColTest <- which(colSums(categorics[is.na(all$SalePrice),])==0)

# removing both together
categorics <- categorics[,-c(zeroVarTrain, zeroColTest)]
```
...
It actually worked very fast. Lasso works great for when you have plenty of predictors by setting regression coefficients for many of them to zero. I want to see how many of the 179 features were used.
...
```{r}
set.seed(11132020)
xgbcv <- xgb.cv( params = default_param, data = dtrain, nrounds = 1000, nfold = 10, showsd = T, stratified = T, print_every_n = 50, early_stopping_rounds = 10, maximize = F)
```
...
xgb_mod <- xgb.train(data = dtrain, params=default_param, nrounds=429)
xgb_pred <- exp(predict(xgb_mod, dtest)) #need to reverse the log to the real values
```

## SVM

Support Vector Machines were developed for classification problems, but they can be used for regression as well. There are two hyperparameters to tune with caret, sigma and C. Kernel can be set to Gaussian radial.

```{r svm-tune, cache=TRUE}
svm_grid <- expand.grid(
    sigma = 10^seq(-5, -2, length.out = 10),
    C = 2^seq(0, 7, length.out = 10))
svm_caret <- train(x=train1, y=all$SalePrice[!is.na(all$SalePrice)], method='svmRadial', trControl= my_control, tuneGrid=svm_grid)
svm_caret$bestTune
min(svm_caret$results$RMSE)
```

Caret tuning produces best RMSE of 0.1139437, when sigma=0.0001 and C=128.

```{r}
# gamma is same as sigma, and cost same as C
svm_mod <- svm(x=train1, y=all$SalePrice[!is.na(all$SalePrice)], kernel =
"radial", gamma=0.0001, cost=128)

svm_pred <- exp(predict(svm_mod, test1))
```

## Averaging predictions

* Lasso CV RMSE = 0.112117
* XGBoost CV RMSE = 0.117025
* SVM CV RMSE = 0.1139437
        
```{r}
las_xgb_svm <- data.frame('Id'=test_Ids, 'SalePrice'=rowMeans(cbind(lasso_pred,xgb_pred, svm_pred)))
write.csv(las_xgb_svm, 'lasso_xgb_svm.csv', row.names = FALSE)
```
