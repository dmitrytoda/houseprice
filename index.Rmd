...
```
Whether it is a good idea to discard the features rejected by boruta is another question. For Lasso it leads to increased CV RMSE. For tree-based algorithms like XGBoost still need to check.
...
I will split the data frame vertically into two: 

1. True numeric (square footage, number of bathrooms etc) - need to be centered and scaled
2. Categorical: remove empty and almost-empty levels, one-hot encode (only for non-ordinals)
        

```{r}
# get all predictors of numeric type
numeric_names <- names(which(sapply(all, is.numeric)))

# manually list ordinals
ordinal_names <- c('LotShape', 'LandSlope', 'OverallQual', 'OverallCond', 'ExterQual', 'BsmtQual', 'BsmtCond', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'HeatingQC', 'KitchenQual', 'Functional', 'FireplaceQu', 'GarageFinish', 'GarageQual', 'PavedDrive')

# remove ordinals from numerics
numeric_names <- numeric_names[!(numeric_names %in% c(ordinal_names, 'SalePrice'))]

# get ALL categories (both ordered and unordered)
categoric_names <- names(all)
categoric_names <- categoric_names[!(categoric_names %in% c(numeric_names, 'SalePrice'))]

# go from names to data frame slices
numerics <- all[,numeric_names]
categorics <- all[,categoric_names]
```
...
By analogy, if some dummy variable (in train and in test set separately) consists of all ones, this does not bring us any useful information either. Some of thise all-one dummy variables may have been created as a side effect of removing almost-empty levels. E.g. Street had 6 x Grvl and 1454 x Pave in the original training set. Now it is all Pave which will become all ones, useless and also forbidding to some algorithms like SVM.

```{r}
# train set
zeroColTrain <- which(colSums(categorics[!is.na(all$SalePrice),])==0)
oneColTrain <- which(colSums(1-categorics[!is.na(all$SalePrice),])==0)

# test set
zeroColTest <- which(colSums(categorics[is.na(all$SalePrice),])==0)
oneColTest <- which(colSums(1-categorics[is.na(all$SalePrice),])==0)

# removing both together
categorics <- categorics[,-c(zeroColTrain, zeroColTest, oneColTrain, oneColTest)]
```
...
I have two vertically sliced data frames now. I will combine them vertically, then split back into train and test horizontally.

```{r}
combined1 <- cbind(numerics, categorics)
train1 <- combined1[!is.na(all$SalePrice),]
test1 <- combined1[is.na(all$SalePrice),]
dim(train1)
```

213 features (SalePrice is separate)
...
It actually worked very fast. Lasso works great for when you have plenty of predictors by setting regression coefficients for many of them to zero. I want to see how many of the 213 features were used.
...
lasso_submit <- data.frame('Id'=test_Ids, 'SalePrice'=lasso_pred)
write.csv(lasso_submit, 'lasso213.csv', row.names = FALSE)
```

Leader board score of 0.12806 with lasso alone.

## XGBoost

XGBoost or extreme gradient boosting is one of the many boosted tree algorithms. It first trains a decision tree, then trains another one to correct the errors of the first one, and so on: a forest but not a random one. 
Getting a final trained model will be a three-step process:

1. Create a search grid to find the best eta, max_depth and min_child_weight for a given large number of trees (1000), using caret
2. Use parameters from step 1 to find the optimal number of trees (it will likely be below 1000)
3. Train a final model using all parameters from steps 1-2 and make a prediction
...
Using native XGBoost interface here

```{r}
# put our testing & training data into two seperates Dmatrixs objects
dtrain <- xgb.DMatrix(data = as.matrix(train1), label= all$SalePrice[!is.na(all$SalePrice)])
dtest <- xgb.DMatrix(data = as.matrix(test1))

# use best hyperparameters from Step 1
default_param<-list(
...
The next step is to do cross validation to determine the best number of rounds (trees) (for the given set of parameters). 
...
xgb_mod <- xgb.train(data = dtrain, params=default_param, nrounds=401)
```

```{r}
xgb_pred <- exp(predict(xgb_mod, dtest)) #need to reverse the log to the real values
xgb_submit <- data.frame('Id'=test_Ids, 'SalePrice'=xgb_pred)
write.csv(xgb_submit, 'xgb.csv', row.names = FALSE)
```

Leader board score of 0.12742, slighly worse than Lasso.

## SVM

Training a support vector machine with hyperparameters suggested in <a href="https://www.kaggle.com/couyang/hybrid-svm-benchmark-approach-0-11180-lb-top-2">Hybrid SVM Benchmark Approach [0.11180] LB: Top 2%</a>, that is, cost=3 and everything else default. The author's input data was actually slighly different because of the NA treatment and feature engineering, but let's see if their setup works for me.

Not doing cross-validation, just training final model right away:

```{r}
svm_mod <- svm(x=train1, y=all$SalePrice[!is.na(all$SalePrice)], cost=3)
```

Predict and sumbit:
```{r}
svm_pred <- exp(predict(svm_mod, test1)) #need to reverse the log to the real values
svm_submit <- data.frame('Id'=test_Ids, 'SalePrice'=svm_pred)
write.csv(svm_submit, 'svm.csv', row.names = FALSE)
```

LB score of 0.14693. Not a good result on its own.
...
```{r}
avg_submit <- data.frame('Id'=test_Ids, 'SalePrice'=rowMeans(cbind(lasso_pred, xgb_pred, svm_pred)))
write.csv(avg_submit, 'lasso_xgb_svm.csv', row.names = FALSE)
```

LB score of 0.12195, better than both Lasso and XGB by themselves.